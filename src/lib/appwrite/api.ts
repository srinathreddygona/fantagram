...
export async function createUserAccount(user: INewUser) {
  try {
    ...
    console.log("newlycreated suer",newUser);
    return newUser;
  } catch (error) {
    console.log("newly created user not getting",error);
    return error;
  }
}
...
export async function signInAccount(user: { email: string; password: string }) {
  try {
    console.log("gettingacount....");
    const session = await account.createEmailPasswordSession(user.email, user.password);
    console.log("Session creeted:",session);

    return session;
  } catch (error) {
    console.log("no session created",error);
  }
}
...
export async function getAccount() {
  ...
  try {
    ...
    console.error("Error fetching account aaaaaah :", error);
...
export async function unfollowUser(followerId: string, followingId: string) {
  try {
    ...
    if (!followRecords.documents.length) {
  ...
  try {
    ...
    const followerUsers = await Promise.all(
      followers.documents.map(async (follow) => {
        return await getUserById(follow.follower);
      })
    );
  ...
  try {
    ...
    const followingUsers = await Promise.all(
      following.documents.map(async (follow) => {
        return await getUserById(follow.following);
      })
    );
